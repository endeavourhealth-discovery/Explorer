...
DELIMITER //
CREATE PROCEDURE getOrgString(IN p_org VARCHAR(1000), IN p_org_tab_name VARCHAR(64), OUT p_orgstring VARCHAR(255))
BEGIN

   CALL storeString(p_org);
   ...
   WHERE EXISTS (SELECT 1 FROM store s WHERE s.code = org_grp.name)");
